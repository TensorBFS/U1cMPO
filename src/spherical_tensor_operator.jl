...
function Xelem_theta_0(M::Integer, l1::Integer, m1::Integer, l2::Integer, m2::Integer)
    return (-1)^(m1) * sqrt((2*l1+1)*(2*l2+1)) * 
    wigner3j(l1, 1, l2, 0, 0, 0) *
    wigner3j(l1, 1, l2, -m1, M, m2) |> Float64
end

function Xelem_theta_pi(M::AbstractFloat, l1::AbstractFloat, m1::AbstractFloat, l2::AbstractFloat, m2::AbstractFloat)
    M, l1, m1, l2, m2 = map(rationalize, [M, l1, m1, l2, m2])
    return (-1)^(l1+l2+1) * (-1)^(m1+1//2) * 
    wigner3j(l1, 1, l2, 0, 0, 0) *
    wigner3j(l1, 1, l2, -m1, M, m2) |> Float64
end
...
function X_theta_0(M::Integer, lmax::Integer)
    ...
    for m1 in lmax:-1:(-lmax)
        m2 = m1 - qn
        if -lmax <= m2 <= lmax
            calc_elem(l1::AbstractFloat, l2::AbstractFloat) = Xelem_theta_0(qn, l1, m1, l2, m2)
            l1s = abs(m1):lmax
            l2s = abs(m2):lmax
...
function X_theta_pi(M::Integer, double_lmax::Integer)
    ...
    for m1 in lmax:-1:(-lmax)
        m2 = m1 - qn
        if -lmax <= m2 <= lmax
            calc_elem(l1::AbstractFloat, l2::AbstractFloat) = Xelem_theta_pi(qn, l1, m1, l2, m2)
            l1s = abs(m1):lmax
            l2s = abs(m2):lmax
            l1_blk = ones(Float64, length(l2s))' .* l1s
            l2_blk = l2s' .* ones(Float64, length(l1s))
